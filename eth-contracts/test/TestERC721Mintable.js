	...
	describe('match erc721 spec', function () {
		...
		it('should return token uri', async function () {
			...
			assert.equal(
				...
				'Token uri should be complete'
			);
		});

		it('should transfer token from one owner to another', async function () {
			await this.contract.transferFrom(account_five, account_six, 4, {
				from: account_five,
			});
			...
			assert.equal(balance6.toString(), '3', 'Should now have 3 tokens');
	...
	describe('have ownership properties', function () {
		...
		it('should fail when minting if address is not contract owner', async function () {
			...
			assert.equal(
				...
				'Should return the error from onlyOwner modifier'
			);
		});
